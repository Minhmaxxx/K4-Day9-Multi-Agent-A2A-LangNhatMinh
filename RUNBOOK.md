...
`--category-language source` preserves the exact `product_category_name` value from the products CSV, which is safest for an exact-match autograder. `english` derives labels from `product_category_name_translation.csv` and should only be used when the grading schema explicitly requests translated labels. Verify and archive a candidate only after you decide it is better than the submitted baseline.
